	...
	while ( !feito ) {
	  ...
	  switch (estado) {
		...
		case 0: {
		  ...
		  switch( caractere ) {
		    ...
		    case '+': {
			  ...
			  case '-': {
				  int nextChar = pegardoBuffer();
				  if (nextChar == '>') { // Operador lambda METODO NOVO
					  tipoAtual = Token.LAMBDA;
					  valorAtual = new Integer(Token.LAMBDA_OP);
					  feito = true;
				  } else {
					  retoneparaBuffer(nextChar);
					  tipoAtual = Token.OP;
		...
		case 38: {
		  ...
		  case 39: {
			  if (Character.isDigit(caractere)) {
				  ...
				  estado = 50; // Novo estado para parte decimal
				  sBuffer.append((char) caractere);
			  } else {
				  retoneparaBuffer(caractere);
		...
		case 49: {
		  ...
		  // Novo estado para parte decimal do float
		  case 50: {
			  if (Character.isDigit(caractere)) {
				  estado = 50;
				  sBuffer.append((char) caractere);
				  ...
				  valorAtual = new Float(sBuffer.toString());
				  feito = true;
			  }
		  } break;
